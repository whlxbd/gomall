...
func (s *ListProductsService) Run(req *product.ListProductsReq) (resp *product.ListProductsResp, err error) {
	...
	_ = pool.Submit(func() {
		klog.Infof("list products: %+v", req)
	})
	...
	if req.CategoryName == "" {
		...
		if err != nil {
			_ = pool.Submit(func() {
				klog.Errorf("get all products failed: %+v", err)
			})
		...
		if err != nil {
			_ = pool.Submit(func() {
				klog.Errorf("get category failed: %+v", err)
			})
		...
		if id == 0 {
			_ = pool.Submit(func() {
				klog.Errorf("category not found: %+v", req.CategoryName)
			})
		...
		if err != nil {
			_ = pool.Submit(func() {
				klog.Errorf("get products by category failed: %+v", err)
			})
